...
{
    NODE *w = treeSearch(root, k);  // 삭제할 노드를 탐색
    if (w == NULL || isExternal(w)) // 키가 없거나 외부 노드인 경우
    {
        return -1; // 삭제 실패 시 -1 반환
    }

    int removedKey = w->key; // 삭제할 키 저장
    NODE *z = w->lChild;     // 왼쪽 자식 노드 저장
    NODE *zs = NULL;
    NODE *y = NULL;
    if (!isExternal(z))
    {
    ...
    {
        zs = reduceExternal(z); // 외부 노드를 줄임
    }
    else // case 2
    {
        y = inOrderSucc(w);             // 중위 후속자 찾기
        z = y->lChild;                  // 중위 후속자의 왼쪽 자식 노드 저장
        w->key = y->key;                // w에 후속자 값을 할당
        zs = reduceExternal(y->lChild); // 후속자를 외부 노드로 줄임
    }
    searchAndFixAfterRemoval(zs->parent); // 균형 조정
...
{
    NODE *parent = w->parent;                                                // 외부 노드의 부모
    NODE *sibling = (w == parent->lChild) ? parent->rChild : parent->lChild; // 형제 노드

    // 부모의 자식을 형제로 바꾸기
    if (isRoot(parent))
    {
        root = sibling;         // 부모가 루트인 경우 형제를 루트로 설정
        sibling->parent = NULL; // 형제의 부모를 NULL로 설정
    }
    else
    {
        NODE *grandparent = parent->parent; // 부모의 부모
        sibling->parent = grandparent;      // 형제의 부모를 할아버지로 설정
        if (parent == grandparent->lChild)
        {
            grandparent->lChild = sibling; // 부모가 할아버지의 왼쪽 자식인 경우
        }
        else
        {
            grandparent->rChild = sibling; // 부모가 할아버지의 오른쪽 자식인 경우
        }
    }

    free(w);      // 외부 노드 메모리 해제
    free(parent); // 부모 노드 메모리 해제

    return sibling; // 형제 노드를 반환하여 이후 균형 조정에 사용
}
...
void searchAndFixAfterRemoval(NODE *w)
{
    ...
    // z의 높은 자식이 y
    if (z->lChild != NULL && z->lChild->height > z->rChild->height)
    {
        y = z->lChild;
    }
    else
    ...
    // y의 두 자식 중 높이가 더 높은 자식이 x, 높이가 같으면 y와 같은 쪽의 자식을 x로 선택
    if (y->lChild != NULL && y->lChild->height > y->rChild->height)
    {
        x = y->lChild;
    }
    else if (y->rChild != NULL && y->lChild->height < y->rChild->height)
    {
        x = y->rChild;
    }
    else
